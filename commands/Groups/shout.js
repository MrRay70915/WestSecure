...
exports.run = async (client, message, args, level) => {
  ...
  .then(async(groupinf) => {
    ...
    try {
        ...
        try {
          await roblox.demote(groupID, rbxID);
        } catch (err) {
        ...
        editmsg.edit({embeds: [embedMaker(`${client.config.emojis.check} Successfully Demoted`, "#3CB371", `You have successfully demoted ${rlbxusername} from ${oldRankName} (${oldRankID}) to ${newRankName} (${newRankID})`)]});
      
         
    } catch (err) {
        
        return editmsg.edit({embeds: [embedMaker("", "RED", `${client.config.emojis.x} There was an error while attempting to demote ${rlbxusername}: ${err}`)]});
    }
...
exports.help = {
    ...
    description: "demote a roblox user in the main specified group",
