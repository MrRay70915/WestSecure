...
module.exports = (client) => {
  ...
  client.WestSecure = {};
  client.WestSecure.lookUp = async function(guild) {
      if (guild == null) return defaults
      return await client.WestSecure.read(guild.id)
  }
  client.WestSecure.read = async function(id) {
              ...
              db.collection(collection).find(query).toArray(async function(err, result) {
                  ...
                  if (result.length < 1) {
                      create(id).then(async _ => {
                          resolve(await client.WestSecure.read(id));
  ...
  checkup = function(guildID, Settings) {
      ...
      client.WestSecure.hardSet(guildID, keep)
  };
  client.WestSecure.edit = async function(guildID, configItem, value) {
      const data = await client.WestSecure.read(guildID)
  ...
  client.WestSecure.hardSet = async function(guildID, wholeSettingsTable) {
      const promise = new Promise((resolve, reject) => {
          MongoClient.connect(url, {
              useUnifiedTopology: true
          }, async function(err, client) {
  ...
  client.WestSecure.add = async function(id, configItem, toAdd) {
      const data = await client.WestSecure.read(id)
      const toEdit = data[configItem]
      if (toEdit.value.includes(toAdd)) return [false, `Attempted to write: ${toAdd} but that value was already present`]
      if (typeof(toEdit.value) !== "object") return [false, "Did you mean 'client.WestSecure.edit'"]
  ...
  client.WestSecure.removeObject = async function(id, configItem, toRemove, objectID) {
      const data = await client.WestSecure.read(id)
      const toEdit = data[configItem]
      if (typeof(toEdit.value) !== "object") return [false, "Did you mean 'client.WestSecure.edit'"]
      const array = []
  ...
  client.WestSecure.remove = async function(id, configItem, toRemove) {
      const data = await client.WestSecure.read(id)
      const toEdit = data[configItem]
      if (typeof(toEdit.value) !== "object") return [false, "Did you mean 'client.WestSecure.edit'"]
      const array = []
  ...
  client.WestSecure.create = create
};