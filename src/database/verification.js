...
module.exports = (client) => {
    ...
    client.database.verify.getDiscordId = function(robloxID){
        let promise = new Promise((resolve, reject) => {
            MongoClient.connect(url,{ useUnifiedTopology: true } , function(err, client) {
              if (!client){return;}
                const db = client.db(dbName);
                ...
                db.collection(collectionName).find(query).toArray(async function(err, result) {
                    ...
                    if(result.length < 1){
                        ...
                        resolve([true, result[0].DiscordID])
                        return client.close()
                    }
                });
            });
    ...
    client.database.verify.event = async function(DiscordTag, RobloxUsername, RobloxID, type, ExtraDetails, server = "Unknown"){
       ...
       // client.channels.fetch(client.config.logChannel).then((c) => {c.send({embeds: [embed]})});
    }
}